# Relation 0007. Perintah doctor dan migrate
...
| `requires` | [0008. SQLite core dan migration runner](../0008-sqlite-core-migrations/index.md) | Indeks build dan konteks spec |

Tidak ada dependency kelompok tambahan dari indeks.

## Konteks prasyarat dari spec utama

> spec 0006 dan 0008 (runner migrasi yang dipanggil `migrate`).
...
|---|---|---|
| `enables` | [0049. Backup](../0049-backup/index.md) | Spec ini harus selesai sebelum konsumen dimulai |

## Relasi nonblocking dan handoff

| Jenis awal | Spec | Cara membaca relasi |
|---|---|---|
| `coordinates` atau `deferred` | [0010. Key provider dan password hashing](../0010-key-provider-password-hashing/index.md) | Mention dalam spec utama bukan otomatis prasyarat. Baca konteks sebelum mengubah graph build. |
| `coordinates` atau `deferred` | [0012. Package config](../0012-config-package/index.md) | Mention dalam spec utama bukan otomatis prasyarat. Baca konteks sebelum mengubah graph build. |

## Prasyarat environment atau manusia

| Jenis | Kebutuhan | Bukti kesiapan |
|---|---|---|
| `environment` | spec 0006 dan 0008 (runner migrasi yang dipanggil migrate). | Dicatat sebelum build atau verify dimulai. |
