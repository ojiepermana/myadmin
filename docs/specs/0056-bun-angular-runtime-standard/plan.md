...
| Build plan          | 0 dari 7 langkah selesai penuh; langkah 1 dan 3 sudah berjalan sebagian (lihat tabel di bawah)                                                                                                   |
| Acceptance criteria | 25 AC: 2 terbukti lokal (AC-4, AC-10), 23 belum terbukti. Angka kanonis tetap `ac-evidence-matrix.md` setelah `bun run matrix:ac` dijalankan ulang.                                              |
...
| --- | --------------------------------------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Mulai dari Bun SQL dan cancellation (area A)                                                  | AC-1, AC-2, AC-3, AC-4                   | Sebagian: AC-4 selesai (probe native tools keluar dari core, `format` opaque, `DatabaseEngine` kanonik tunggal di `@myadmin/kernel`). AC-1, AC-2, AC-3 belum dimulai.                                             |
| 2   | Lanjutkan Bun I/O (area B)                                                                    | AC-5, AC-6, AC-7                         | Tidak dinyatakan                                                                                                                                                                                                  |
| 3   | Pecah ownership lifecycle Elysia (area D)                                                     | AC-8, AC-9, AC-10, AC-11                 | Sebagian: AC-10 selesai (siklus `apps/server` dan `apps/cli` putus lewat `@myadmin/runtime-assets`, enam aturan dependency cruiser baru aktif, seluruh siklus lama dibersihkan). AC-8, AC-9, AC-11 belum dimulai. |
...
| [AC-4](test.md#ac-4)   | `database-core` bebas I/O runtime; probe tools keluar core; `DatabaseEngine` kanonik tunggal          | `IT-0056-AC4`                         | Terbukti lokal: `bun run test` (624 pass, 18 skip, 0 fail), `tests/quality/runtime-standard-0056.test.ts`              |
...
| [AC-10](test.md#ac-10) | Siklus import `apps/server` dan `apps/cli` putus; dependency cruiser menegakkan aturan boundary       | `IT-0056-AC10`                        | Terbukti lokal: `bun run check:boundaries` (0 violations, 425 modul) dan `tests/quality/runtime-standard-0056.test.ts` |
