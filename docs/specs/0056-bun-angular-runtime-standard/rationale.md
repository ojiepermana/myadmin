...
Di sisi server, `apps/server/src/app.ts` masih memuat composition root, lifecycle cleanup, registrasi route, timer, dan assembly fixture yang sebagian besar diulang. Di sisi Angular, signals telah dipakai dan aplikasi sudah berjalan zoneless (build web tidak memuat polyfill zone.js, default Angular 22), tetapi read model masih bercampur dengan `firstValueFrom`, subscription manual, loading flag, counter stale, polling, dan stop map. Keadaan zoneless itu belum menjadi kebijakan yang diuji: tidak ada gate per feature, sehingga sebuah feature bisa saja lulus karena kebetulan urutan render. Aturan resource yang menjadi sumber kebenaran juga belum ada. OpenAPI, validator registry, Elysia route, generated types, SDK path, dan WebSocket event juga belum memiliki traceability tunggal.
...
## Penyesuaian 2026-08-29 setelah review implementasi

Spec ini disesuaikan sebelum ratifikasi berdasarkan review implementasi menyeluruh (arsitektur, kontrak, web, keamanan, kualitas test, dan audit evidence) pada tanggal yang sama. Enam penyesuaian, dengan alasannya:

1. **Test ID mengikuti konvensi bertipe.** Format lama `S0056-NN` tidak cocok dengan pola `JENIS-NNNN-ACn` yang dibaca `scripts/quality/generate-ac-evidence-matrix.ts`, sehingga seluruh bukti 0056 tidak akan pernah masuk matrix. `test.md` ditulis ulang mengikuti format rumah yang dipakai 55 spec lain.
2. **AC dipecah granular, satu kepedulian per AC.** AC bundel (satu AC menuntut lima jenis bukti berbeda) dijamin berstatus parsial menahun, mengulang masalah yang sudah terlihat pada matrix saat ini. Sepuluh AC lama menjadi dua puluh lima AC per area child.
3. **Prasyarat dua tingkat.** Daftar prasyarat lama memuat spec 0053, 0054, dan 0055 yang belum dibangun, sementara rollout menyuruh mulai dari Bun SQL. Kontradiksi itu diselesaikan: standar internal (child A, B, C, D, F) tidak menunggu; hanya langkah cutover child E yang menunggu ketiga spec itu plus evidence v1 yang sehat.
4. **Mekanisme versi ditegaskan tunggal.** Base URL yang ada sudah memuat `v1` di path (`/api/v1`, keputusan spec 0005), sehingga menambah header versi menciptakan dua mekanisme yang tampak bertabrakan. Keputusan: header `X-MyAdmin-API-Version` adalah satu satunya pemilih kontrak; segmen `v1` di path dibekukan sebagai base path. Alternatif path baru `/api/v2` ditolak karena memaksa migrasi base URL SDK, struktur folder kontrak, dan seluruh endpoint yang tidak ikut dimigrasikan; blast radius itu tidak sebanding untuk produk single binary yang server dan SPA nya selalu rilis bersama. Nilai nyata header pasca cutover adalah menolak tab SPA basi dari rilis lama dengan error yang jelas.
5. **Premis zoneless dikoreksi.** Aplikasi sudah berjalan tanpa zone.js; child C dibingkai ulang dari migrasi menjadi peresmian kebijakan plus gate test per feature. Ini mengubah sifat pekerjaannya dari perubahan runtime berisiko menjadi penambahan bukti.
6. **Scope child diperluas dengan temuan review yang belum tertampung.** Child A menampung netralisasi sisa kebocoran `database-core` (probe native tools, format backup opaque). Child D menampung konsolidasi helper keamanan HTTP yang terduplikasi dua belas kali (drift nyata sudah terjadi) dan pemutusan siklus import `apps/server` dengan `apps/cli` beserta perluasan rule dependency cruiser. Child E menambah nama query parameter dan schema requestBody ke pemeriksaan registry (kelas bug `pageSize` versus `page-size` yang lolos di v1). Child F menetapkan tiga slice aksesibilitas pertama dari temuan konkret (modal drop tanpa focus trap, edit sel hanya lewat double click, tabindex grid tanpa roving).
