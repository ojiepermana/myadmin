...
1. [x] Definisikan schema TypeBox plus default dan flag sensitif di `config/schema/`, memenuhi **AC-1**.
2. [x] Bangun loader (flag, env mapping, TOML, merge berprioritas, immutability, metadata sumber) di `config/loaders/`, memenuhi **AC-2**, **AC-3**, **AC-4**.
3. [x] Integrasikan redaction (spec 0011) untuk dump, memenuhi **AC-5**.
4. [x] Pindahkan pembacaan host/port/dataDir CLI (spec 0006) ke loader ini; daftarkan doctor check, memenuhi **AC-2**, **AC-6**.
5. [x] Unit test lengkap, memenuhi **AC-7**.
