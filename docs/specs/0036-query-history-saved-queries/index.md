...
- [**AC-4**](test.md#ac-4): saved queries CRUD: `GET/POST/PATCH/DELETE /query/saved`; nama wajib dan unik per user (409 bila bentrok), SQL wajib, konteks opsional (connectionId, database); membuka saved query membuat tab baru dengan konteksnya.
...
**Data model sketch**: memakai `query_history` dan `saved_queries` (spec 0008) tanpa perubahan.
...
| /query/saved | GET/POST | name, sql, connectionId?, database? | item(s) | pemilik | 409 nama |
...
1. Tambah operasi history dan saved ke kontrak, regenerasi, contract test.
2. Use case dan endpoint server (filter berparameter, kepemilikan, retensi saat penulisan, hapus semua), memenuhi **AC-1** sampai **AC-4**, **AC-6**.
3. UI halaman query-history dua tab plus panel cepat di editor plus simpan cepat, memenuhi **AC-5**, **AC-7**.
